...
const clip=(text,max=220)=>String(text||'').length>max?`${String(text).slice(0,max-1).trim()}…`:String(text||'');

function currentVerseTranslation(){try{const value=localStorage.getItem(TRANSLATION_KEY);return ['bsb','kjv','custom'].includes(value)?value:'bsb'}catch{return'bsb'}}
function customTranslationMap(){try{return JSON.parse(localStorage.getItem(CUSTOM_TRANSLATION_KEY)||'{}')||{}}catch{return{}}}
function activeVerseText(verse){return verseText(verse,currentVerseTranslation(),customTranslationMap())}
...
function versePool(spec={}){
  let pool=VERSES;
  if(spec.scope&&!spec.scope.all){const allowed=new Set(booksForScope(spec.scope).map(item=>item.n));pool=pool.filter(verse=>allowed.has(verse.bn))}
  if(Array.isArray(spec.verseScope)&&spec.verseScope.length)pool=pool.filter(verse=>(verse.themes||[]).some(theme=>spec.verseScope.includes(theme)));
  return pool.length?pool:VERSES;
}
function verseBookOptions(target,pool){
  const available=[...new Map(pool.map(verse=>[verse.bn,book(verse.bn)])).values()].filter(Boolean),others=sample(available.filter(item=>item.n!==target.bn),3);
  if(others.length<3)others.push(...wrongBooks(book(target.bn),others,3-others.length));
  return [book(target.bn),...others].filter(Boolean).map(item=>({value:String(item.n),label:item.name}));
}
function questionVerseBook(spec,variant='verse-book'){
  const pool=versePool(spec),target=pick(pool),text=activeVerseText(target),context=variant==='guess-book'?`${target.speaker} → ${target.recipient}: `:'';
  return mc(`${variant}-${target.ref}`,`${context}Which book contains this passage? “${clip(text,210)}”`,String(target.bn),verseBookOptions(target,pool),`${target.ref}. ${target.note}`,{book:target.bn,verseRef:target.ref});
}
function questionVerseTheme(spec){
  const pool=versePool(spec),target=pick(pool),themes=(target.themes||[]).filter(theme=>theme!=='fruitlist'),answer=pick(themes.length?themes:['faith']),wrong=sample(VERSE_THEMES.filter(theme=>theme!==answer&&theme!=='fruitlist'&&!themes.includes(theme)),3),options=[answer,...wrong].map(theme=>({value:theme,label:theme.replace(/\b\w/g,m=>m.toUpperCase())}));
  return mc(`verse-theme-${target.ref}-${answer}`,`Which theme is tied to ${target.ref}? “${clip(activeVerseText(target),180)}”`,answer,options,`${target.ref} is indexed under ${(target.themes||[]).filter(t=>t!=='fruitlist').join(', ')}. ${target.note}`,{book:target.bn,verseRef:target.ref});
}
function fillWords(text){return String(text||'').split(/\s+/).map((raw,index)=>({raw,index,word:raw.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}'’-]+$/gu,'')})).filter(item=>item.word.length>=4)}
function questionVerseFill(spec){
  const pool=versePool(spec);let target=pick(pool),text=activeVerseText(target),words=fillWords(text);for(let tries=0;words.length<3&&tries<8;tries++){target=pick(pool);text=activeVerseText(target);words=fillWords(text)}
  const chosen=pick(words),tokens=text.split(/\s+/),raw=tokens[chosen.index],answer=chosen.word;tokens[chosen.index]=raw.replace(answer,'_____');
  const distractors=sample([...new Set(fillWords(text).map(item=>item.word).filter(word=>word!==answer))],3),options=[answer,...distractors].map(word=>({value:word,label:word}));
  return mc(`verse-fill-${target.ref}-${chosen.index}`,`Complete ${target.ref}: “${clip(tokens.join(' '),230)}”`,answer,options,`${target.ref}: ${text}`,{book:target.bn,verseRef:target.ref});
}
function questionVerseJumble(spec,hard=false){
  const pool=versePool(spec);let target=pick(pool),words=activeVerseText(target).split(/\s+/).filter(Boolean);for(let tries=0;words.length<6&&tries<8;tries++){target=pick(pool);words=activeVerseText(target).split(/\s+/).filter(Boolean)}
...
function generate(engine,spec){
  ...
  if(engine==='hook'||engine==='content-drill')return questionHook(spec);
  if(engine==='guess-book')return questionVerseBook(spec,'guess-book');
  ...
  if(engine==='jumble')return questionVerseJumble(spec,false);
  if(engine==='jumble-hard')return questionVerseJumble(spec,true);
  if(engine==='verse-book')return questionVerseBook(spec);
...
const gameFamilyById=id=>PRACTICE_GAME_FAMILIES.find(([game])=>game===id)||null;
...
function createRun(spec,{arcade=false,game=null,scope=null}={}){
  ...
  const family=arcade?gameFamilyById(game):null;if(arcade&&!family)throw new Error(`Unknown Arcade game: ${game}`);
  const runSpec=arcade?{id:null,name:family[1],game,engines:[game],count:game==='survival'?40:game==='clock'?20:10,scope:scopeById(scope).scope,format:game}:spec;
  ...
  else if(format==='pairs-number')questions=[pairsQuestion(runSpec,true)];
  else{
...
export function practiceRunView({params,esc}){
  ...
  try{if(levelId){const level=levelById(levelId);if(!level)return'<p class="notice">Practice level not found.</p>';session=createRun(level)}else if(arcadeGame)session=createRun(null,{arcade:true,game:arcadeGame,scope});else return''}catch(error){return `<p class="notice"><strong>Practice engine unavailable:</strong> ${esc(error?.message||'Unknown error')}</p>`}
...
export function finishPracticeRun(form){
  ...
  const missedMarkup=missed.length?`<details class="missed-review"><summary>Review ${missed.length} missed ${missed.length===1?'answer':'answers'}</summary><ol>${missed.map(item=>`<li><strong>${escHtml(item.question)}</strong><p>${escHtml(item.want)}</p></li>`).join('')}</ol></details>`:'<p>No missed answers in this round.</p>';
  return{ok:true,complete:true,html:`<div class="round-result"><p class="eyebrow">Round complete</p><h2>${verdict}</h2><div class="round-result__metrics"><strong>${correct}/${total}</strong><span>${stars}★</span><span>+${result.xpGained} XP</span><span>${escHtml(result.rank.name)}</span></div>${result.rankAdvanced?`<p class="notice"><strong>Rank advanced:</strong> ${escHtml(result.rank.name)}</p>`:''}${missedMarkup}<div class="round-result__actions"><a class="button" href="${session.arcade?'/practice?mode=arcade':'/practice?mode=campaign'}">Back</a>${session.spec.id?`<a class="button button--primary" href="/practice?play=${session.spec.id}">Play again</a>`:`<a class="button button--primary" href="/practice?arcade=${encodeURIComponent(session.game)}&scope=all">Play again</a>`}</div></div>`};
}
