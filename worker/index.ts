...
function adminAuthorized(request:Request,env:Env){const configured=String(env.FEEDBACK_ADMIN_TOKEN||'');if(!configured)return false;return (request.headers.get('authorization')||'')===`Bearer ${configured}`}
...
async function postAdminFeedbackResponse(request:Request,env:Env){
  if(!adminAuthorized(request,env))return bad('Unauthorized',401);let body:any;try{body=await request.json()}catch{return bad('Invalid JSON')}
  ...
  if(!result.ok)return bad(result.reason==='not-found'?'Feedback not found':'Feedback id is required',result.reason==='not-found'?404:400);return json(result);
}
