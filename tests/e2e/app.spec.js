...
async function openStudyGuide(page){const button=page.locator('.study-focus__utilities [data-study-guide]');await expect(button).toBeVisible();await button.click()}
...
test('new lesson uses multi-scene cards with hidden drawers glossary and multiple games',async({page})=>{
  ...
  await expect(page.locator('.scene-rail a')).toHaveCountGreaterThan?.catch(()=>{});
  ...
  const sceneCount=await page.locator('.scene-rail a').count();
  expect(sceneCount).toBeGreaterThanOrEqual(8);
  await noSeriousA11y(page);
});

test('every unit exposes integral mastery and every course has a capstone',async({page})=>{
  ...
  for(const course of data.courses)expect(data.activities.some(activity=>activity.courseId===course.id&&activity.masteryType==='course-capstone')).toBeTruthy();
});
...
test('Unit 0 remains replayable and does not mutate scored progress',async({page})=>{
...
test('Study Focus fits viewport and rich checks do not regress to generic selects',async({page})=>{
  ...
  expect(folio).toBeTruthy();expect(scene).toBeTruthy();expect(nav).toBeTruthy();
  ...
  const data=await catalog(page),lesson=data.lessons.find(item=>item.id==='c2-passover');
  const practiceScene=2+(lesson.body?.length||0)+(lesson.simple?1:0)+(lesson.visual?1:0)+1;
  await page.goto(`/course?unit=c2.exodus&lesson=c2-passover&scene=${practiceScene}`);
...
test('theme packages still persist',async({page})=>{
...
test('account sync stays secondary and mobile-safe',async({page})=>{
  await page.setViewportSize({width:390,height:844});
  await page.goto('/home');
...
test('Bible owns a 66-book shelf and native reference route',async({page})=>{
...
test('Guide remains bounded and protects mastery answers',async({page})=>{
  await page.goto('/home');
  ...
  const data=await catalog(page),mastery=data.activities.find(activity=>activity.masteryType==='unit-mastery');
...
test('offline shell includes the multi-course curriculum and bounded study surfaces',async({page,context,browserName})=>{
