...
test.describe('v7 interactive prototype gate',()=>{
  ...
  test('switches among all three directions and all required surfaces',async({page})=>{
    const switcher=page.locator('.direction-switcher');
    for(const direction of ['A','B','C']){
      await switcher.getByRole('button',{name:new RegExp(`^${direction}`)}).click();
  ...
  test('lesson scenes, practice, Bible, Topics, and Theologian are interactive',async({page})=>{
    ...
    await page.getByRole('navigation',{name:'Prototype pages'}).getByRole('button',{name:'Bible',exact:true}).click();
    ...
    await page.getByRole('navigation',{name:'Prototype pages'}).getByRole('button',{name:'Topics',exact:true}).click();
    ...
    await page.getByRole('navigation',{name:'Prototype pages'}).getByRole('button',{name:'Theologian',exact:true}).click();
