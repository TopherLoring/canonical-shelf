...
function bibleNav(active){
  const modes=[['shelf','01','Bookshelf'],['books','02','Books & groups'],['reader','03','Bible reader'],['timeline','04','Canon & timeline']];
...
function bibleContextHref(params,changes={}){const next=new URLSearchParams(params);if(!next.has('view'))next.set('view','shelf');for(const [key,value] of Object.entries(changes)){if(value===null||value===undefined||value==='')next.delete(key);else next.set(key,String(value))}return `/bible?${next.toString()}`}
...
function bookNotes(book,chapter,esc){
  ...
  return `<aside class="library-reader-panel" aria-label="Book Notes"><header><p class="eyebrow">Reading context</p><h2>Book Notes</h2><div class="session-pane-actions"><button type="button" data-journal-open aria-haspopup="dialog" aria-controls="personal-study-panel">Journal Notes</button><button type="button" data-feedback-open aria-haspopup="dialog" aria-controls="feedback-panel">Feedback</button></div><p class="session-context-note">Your journal and feedback are tied to the current Bible reading; technical location data stays internal.</p></header>${module('At a glance',`<p><strong>${esc(book.name)}</strong> · ${book.ch} chapter${book.ch===1?'':'s'}</p><p>${esc(book.syn||book.hook||'')}</p>`,true)}${module('People & setting',`<p>${esc((book.people||[]).join(' · ')||'People and setting vary across the book.')}</p><p>${esc(era?.name||book.era||'Broad historical setting')} · ${esc(range(book.setA,book.setB))}</p>`)}${module('Group & themes',`<p><strong>${esc(category.name)}</strong></p><p>${esc(category.blurb)}</p>${themes.length?`<ul>${themes.map(theme=>`<li>${esc(theme)}</li>`).join('')}</ul>`:''}`)}${module('Where to start',`<p>${esc(book.read||`Continue with ${book.name} ${chapter}.`)}</p>`)}${module('Reader links',`<p><a href="/bible?book=${book.n}&profile=1">Full book profile</a></p><p><a href="/bible?view=shelf">Back to Bookshelf</a></p>`)} </aside>`;
}
...
function reader(text,bn,chapter,start,end,esc){
  ...
  const shown=start?all.filter(row=>row.verse>=start&&row.verse<=end):all,previous=chapter>1?`/bible?book=${bn}&chapter=${chapter-1}`:bn>1?`/bible?book=${bn-1}&chapter=${chapterCount(text,bn-1)}`:null,next=chapter<count?`/bible?book=${bn}&chapter=${chapter+1}`:bn<66?`/bible?book=${bn+1}&chapter=1`:null;
  return `<section class="library-reader-layout"><article class="reader scripture"><div class="reader-utility"><a href="/bible?book=${bn}&profile=1">← ${esc(book.name)} profile</a><span class="badge">Berean Standard Bible · BSB</span></div><h1>${esc(book.name)} ${chapter}${start?`:${start}${end!==start?`–${end}`:''}`:''}</h1><div class="chapter-nav">${previous?`<a class="button" href="${previous}">← Previous</a>`:'<span></span>'}<label>Chapter <select id="chapter-jump" data-book="${bn}">${Array.from({length:count},(_,i)=>`<option value="${i+1}" ${i+1===chapter?'selected':''}>${i+1}</option>`).join('')}</select></label>${next?`<a class="button" href="${next}">Next →</a>`:'<span></span>'}</div><div class="verses">${shown.map(row=>`<p id="v${row.verse}"><sup>${row.verse}</sup> ${esc(row.text)}</p>`).join('')}</div><footer class="reader-footer"><a href="/bible?book=${bn}&profile=1">Book details</a><a href="/bible?view=shelf">Bookshelf</a></footer></article>${bookNotes(book,chapter,esc)}</section>`;
}
...
export function bibleView(text,params,esc){
  ...
  const content=view==='books'?booksView(params,esc):view==='timeline'?timeline(params,esc):view==='reader'?readerLauncher(text,esc):shelf(params,esc),header=view==='shelf'?bibleLandingHeader():bibleStandardHeader(),background=`${header}${bibleNav(view)}${content}`;
