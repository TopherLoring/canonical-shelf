...
function courseCard(data,state,course,esc){
  ...
  return `<article class="course-journey-card" data-course-state="${p.done===p.total&&p.total?'complete':p.done?'in-progress':'not-started'}"><header><span>Course ${course.sequence}</span><span>${esc(course.level==='advanced'?'Deeper study':'Biblical Literacy Core')}</span></header><h2><a href="/course?course=${encodeURIComponent(course.id)}">${esc(course.title)}</a></h2><p>${esc(course.scope)}</p><div class="course-journey-card__meta"><span>${units.length} units</span><span>${ids.length} activities</span>${reviewDue?`<span class="state-due">${reviewDue} review${reviewDue===1?'':'s'} due</span>`:''}</div><div class="progress" aria-hidden="true"><span style="width:${p.pct}%"></span></div><footer><span>${p.done}/${p.total} · ${p.pct}%</span><a href="${next?activityHref(next):`/course?course=${encodeURIComponent(course.id)}`}">${next?'Continue':'Review course'} →</a></footer></article>`;
}
