...
function renderCourseChoice(course, index, esc, isSelected) {
  return `<button class="course-choice" type="button" data-course-choice="${esc(course.id)}" aria-selected="${isSelected}" ><span class="course-choice__roman">${esc(course.sequence||index+1)}</span><span><strong>${esc(course.shortTitle||course.title)}</strong><small>${esc(course.scope||'')}</small></span><span class="course-choice__arrow" aria-hidden="true">›</span></button>`;
}
...
function volumeLandingView({data,state,esc}){
  ...
  return `<section class="course-volume-landing"><header class="course-volume-heading"><p class="eyebrow">Guided learning · ${courses.length}-course collection</p><h1>Course</h1><p class="lede">Open the curriculum as a set of ${courses.length} volumes. Each course has its own purpose, ordered units, scored activities, and mastery work; completed material remains available for review.</p></header><div class="course-volume-shelf" aria-label="${courses.length} Canonical Shelf courses">${volumes}</div><section class="course-volume-summary"><div><p class="eyebrow">${currentStats?.started?'Continue':'Suggested starting point'}</p><h2>${esc(current?.title||'Foundations')}</h2><p>${esc(current?.scope||'Begin with the orientation and foundational reading skills.')}</p></div><div><strong>${pct}%</strong><p>${done}/${total} scored activities</p>${current?`<a class="button button--primary" href="/course?course=${encodeURIComponent(current.id)}">Open volume →</a>`:''}</div></section></section>`;
}
