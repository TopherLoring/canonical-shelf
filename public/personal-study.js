...
function contextLabel(){
  if(location.pathname==='/course')return document.querySelector('.study-focus__identity strong')?.textContent?.trim()||'Current study activity';
  if(location.pathname==='/bible')return document.querySelector('.reader.scripture h1')?.textContent?.trim()||document.querySelector('[data-book-drawer] h2')?.textContent?.trim()||'Current Bible reading';
  if(location.pathname==='/topics')return document.querySelector('.topic-reference-page h1')?.textContent?.trim()||'Current Topic';
  return'Current study activity';
}

function contextDescription(){
  if(location.pathname==='/bible')return'This journal entry is tied to your current Bible reading.';
  if(location.pathname==='/topics')return'This journal entry is tied to the current Topic.';
  return'This journal entry is tied to the current study activity.';
}
...
async function loadJournal(){
  currentKey=activityKey();if(!currentKey)return false;
...
async function openPanel(trigger=openButton){
  lastTrigger=trigger||openButton;if(!await loadJournal())return;
