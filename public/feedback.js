...
const existingFeedbackId=()=>{try{return localStorage.getItem(ANON_KEY)||''}catch{return''}};
...
function requestHeaders({json=false,createId=false}={}){
  const id=createId?ensureFeedbackId():existingFeedbackId();
...
function setReviewContext(detail){
  ...
  pendingContext={
    kind:'theologian-response',action:clip(detail.action,32)||'review',question:clip(detail.question,1600),answer:clip(detail.answer,9000),
    mode:clip(detail.mode,32),model:clip(detail.model,160),policyVersion:typeof detail.policyVersion==='number'?detail.policyVersion:clip(detail.policyVersion,32),
  ...
  if(contextPreview){
    ...
    contextPreview.textContent=detail.action==='disagree'
      ?'Your disagreement will include the specific Theologian question, answer, and visible evidence metadata for review.'
      :'This review request will include the specific Theologian question, answer, and visible evidence metadata.';
  }
  if(messageInput)messageInput.placeholder=detail.action==='disagree'
    ?'Optional: add the interpretation, evidence, or perspective you want considered.'
    :'Optional: add what seems incorrect, unsupported, incomplete, or too certain.';
  return true;
}
...
function inboxMarkup(items){
  ...
  return items.map(item=>{
    const response=item.reviewerResponse
      ?`<div class="feedback-thread__response"><strong>Canonical Shelf response</strong><p>${esc(item.reviewerResponse)}</p>${item.respondedAt?`<small>${esc(new Date(item.respondedAt).toLocaleString())}</small>`:''}</div>`:'';
...
async function loadInbox(){
  ...
  if(!existingFeedbackId()){
    inbox.innerHTML='<p class="meta">No feedback or review threads are linked to this browser yet.</p>';
    if(inboxStatus)inboxStatus.textContent='';
    return;
  }
  if(inboxStatus)inboxStatus.textContent='Checking for replies…';
  try{
    const response=await fetch('/api/feedback',{headers:requestHeaders()});
...
async function send(payload){
  const response=await fetch('/api/feedback',{method:'POST',headers:requestHeaders({json:true,createId:true}),body:JSON.stringify(payload)});
  if(!response.ok){
...
form?.addEventListener('submit',async event=>{
  ...
  const payload={
    category:pendingContext?'theology':String(data.get('category')||'other'),message:String(data.get('message')||'').trim(),contact:String(data.get('contact')||'').trim(),
    route:routeContext(),clientCreatedAt:new Date().toISOString(),...(pendingContext?{reviewReason:String(data.get('reviewReason')||'').trim(),context:pendingContext}:{})
  };
