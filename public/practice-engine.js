...
function questionNext(spec,direction=1){
  const pool=booksForScope(spec.scope).filter(item=>direction>0?item.n<66:item.n>1),source=pick(pool),target=book(source.n+direction),options=[target,...wrongBooks(target,[source],3)].map(item=>({value:String(item.n),label:item.name}));
...
function questionGap(spec){
  const pool=booksForScope(spec.scope),min=pool[0].n,max=pool.at(-1).n,start=Math.max(min,Math.min(max-4,pick(pool).n)),run=Array.from({length:Math.min(5,max-start+1)},(_,i)=>book(start+i)).filter(Boolean);if(run.length<3)return questionNext(spec,1);const hole=1+Math.floor(Math.random()*(run.length-2)),target=run[hole],strip=run.map((item,index)=>index===hole?'?':item.name).join(' → '),options=[target,...wrongBooks(target,run,3)].map(item=>({value:String(item.n),label:item.name}));return mc(`gap-${target.n}`,`${strip}\nWhich book fills the gap?`,String(target.n),options,`${target.name} is book ${target.n}, between ${run[hole-1].name} and ${run[hole+1].name}.`,{book:target.n})}
function questionBeforeAfter(spec){
  const pool=booksForScope(spec.scope);let a=pick(pool),b=pick(pool);while(a.n===b.n)b=pick(pool);const target=a.n<b.n?'before':'after';return mc(`ba-${a.n}-${b.n}`,`Does ${a.name} come before or after ${b.name} on the canonical shelf?`,target,[{value:'before',label:'Before'},{value:'after',label:'After'}],`${a.name} is book ${a.n}; ${b.name} is book ${b.n}.`,{book:a.n})}
function questionMisfit(spec){
  const pool=booksForScope(spec.scope);if(pool.length<4)return questionBeforeAfter(spec);const start=Math.max(pool[0].n,Math.min(pool.at(-1).n-3,pick(pool).n)),run=Array.from({length:4},(_,i)=>book(start+i)).filter(Boolean),slot=1+Math.floor(Math.random()*2),original=run[slot],intruder=pick(LIBRARY_BOOKS.filter(item=>!run.some(r=>r.n===item.n)&&item.cat!==original.cat)),display=run.map((item,index)=>index===slot?intruder:item);return mc(`misfit-${start}-${intruder.n}`,'One book breaks this consecutive shelf run. Which is the misfit?',String(intruder.n),display.map(item=>({value:String(item.n),label:item.name})),`The intended run is ${run.map(item=>item.name).join(' → ')}.`,{book:intruder.n})}
function questionCategory(spec){const target=pick(booksForScope(spec.scope));return mc(`cat-${target.n}`,`Which canonical group contains ${target.name}?`,target.cat,shuffle(CATEGORY_ORDER).slice(0,5).includes(target.cat)?shuffle(CATEGORY_ORDER).slice(0,5).map(key=>({value:key,label:CATEGORIES[key].name})):[{value:target.cat,label:CATEGORIES[target.cat].name},...sample(CATEGORY_ORDER.filter(key=>key!==target.cat),4).map(key=>({value:key,label:CATEGORIES[key].name}))],`${target.name} belongs to ${CATEGORIES[target.cat].name}. ${CATEGORIES[target.cat].blurb}`,{book:target.n})}
function questionOdd(){const category=pick(CATEGORY_ORDER.filter(key=>LIBRARY_BOOKS.filter(item=>item.cat===key).length>=3)),members=sample(LIBRARY_BOOKS.filter(item=>item.cat===category),3),intruder=pick(LIBRARY_BOOKS.filter(item=>item.cat!==category));return mc(`odd-${category}-${intruder.n}`,`Three are ${CATEGORIES[category].name}. Which one is not?`,String(intruder.n),shuffle([...members,intruder]).map(item=>({value:String(item.n),label:item.name})),`${intruder.name} belongs to ${CATEGORIES[intruder.cat].name}.`,{book:intruder.n})}
function questionBounds(){const category=pick(CATEGORY_ORDER.filter(key=>LIBRARY_BOOKS.filter(item=>item.cat===key).length>=2)),members=LIBRARY_BOOKS.filter(item=>item.cat===category),first=Math.random()<.5,target=first?members[0]:members.at(-1);return mc(`bounds-${category}-${first?'first':'last'}`,`Which book ${first?'opens':'closes'} ${CATEGORIES[category].name}?`,String(target.n),[target,...wrongBooks(target,members,3)].map(item=>({value:String(item.n),label:item.name})),`${target.name} is the ${first?'first':'last'} book in ${CATEGORIES[category].name}.`,{book:target.n})}
function questionHook(spec){const target=pick(booksForScope(spec.scope));return mc(`hook-${target.n}`,`Which book is summarized by “${target.hook}”?`,String(target.n),[target,...wrongBooks(target,null,3)].map(item=>({value:String(item.n),label:item.name})),`${target.name}: ${target.syn}`,{book:target.n})}
function questionPeople(spec){const target=pick(booksForScope(spec.scope).filter(item=>item.people?.length));return mc(`people-${target.n}`,`Which book includes ${target.people.slice(0,Math.min(3,target.people.length)).join(', ')}?`,String(target.n),[target,...wrongBooks(target,null,3)].map(item=>({value:String(item.n),label:item.name})),`${target.name} includes ${target.people.join(', ')}.`,{book:target.n})}
function questionSynopsis(spec){const target=pick(booksForScope(spec.scope));return mc(`syn-${target.n}`,`Which book fits this plot movement? “${target.syn}”`,String(target.n),[target,...wrongBooks(target,null,3)].map(item=>({value:String(item.n),label:item.name})),`${target.name}: ${target.hook}.`,{book:target.n})}

function sequenceQuestion(spec){
  const pool=booksForScope(spec.scope),size=Math.min(Number(spec.size||5),pool.length);let ordered;
...
function generate(engine,spec){
  ...
  if(engine==='category')return questionCategory(spec);
  if(engine==='odd')return questionOdd(spec);
...
function levelById(id){return PRACTICE_LEVELS.find(level=>level.id===id)||null}
function scopeById(id){return PRACTICE_SCOPES.find(scope=>scope.id===id)||PRACTICE_SCOPES[0]}
function gameFamilyById(id){return PRACTICE_GAME_FAMILIES.find(([game])=>game===id)||PRACTICE_GAME_FAMILIES[0]}

function starsFromRatio(ratio){return ratio>=.95?4:ratio>=.82?3:ratio>=.68?2:ratio>=.55?1:0}
function createRun(spec,{arcade=false,game=null,scope=null}={}){
  ...
  const format=runSpec.format||runSpec.game,engineList=runSpec.engines?.length?runSpec.engines:[runSpec.game];
  let questions=[];
  if(format==='shelf')questions=[shelfQuestion(runSpec)];
    ...
    for(let index=0;index<count;index++){const engine=engineList[index%engineList.length];questions.push(generate(engine,runSpec))}
  }
  const session={runId,spec:runSpec,questions,arcade,game:runSpec.game,startedAt:Date.now(),seconds:runSpec.seconds||60,lives:runSpec.lives||3};
  sessions.set(runId,session);return session;
}

function inputName(index,suffix='answer'){return `q${index}-${suffix}`}
...
export function practiceRunView({params,esc}){
  ...
  if(levelId){const level=levelById(levelId);if(!level)return'<p class="notice">Practice level not found.</p>';session=createRun(level)}
  else if(arcadeGame)session=createRun(null,{arcade:true,game:arcadeGame,scope});
  else return'';
...
function valuesForSequence(form,index,count){return Array.from({length:count},(_,i)=>String(new FormData(form).get(inputName(index,`p${i}`))||''))}
function gradeQuestion(form,question,index){const fd=new FormData(form);if(question.shape==='mc')return String(fd.get(inputName(index))||'')===String(question.answer);if(question.shape==='sequence')return valuesForSequence(form,index,question.answer.length).every((value,i)=>value===question.answer[i]);if(question.shape==='shelf')return question.slots.every(slot=>String(fd.get(inputName(index,`slot-${slot}`))||'')===String(question.answer[String(slot)]));if(question.shape==='bins')return question.items.every(item=>String(fd.get(inputName(index,`bin-${item.value}`))||'')===question.answer[item.value]);if(question.shape==='pairs')return question.items.every(item=>String(fd.get(inputName(index,`pair-${item.value}`))||'')===question.answer[item.value]);return false}
...
export function finishPracticeRun(form){
  ...
  const graded=session.questions.map((question,index)=>({question,correct:gradeQuestion(form,question,index)}));
  let considered=graded;
  ...
  const result=recordPracticeResult({levelId:session.spec.id,game:session.game,correct,total,stars,streak:bestStreak,missed,learnedBooks,timed:session.spec.format==='clock',survival:session.spec.format==='survival',arcade:session.arcade});
...
let timerId=null;
export function activatePracticeRun(root=document){if(timerId){clearInterval(timerId);timerId=null}const form=root.querySelector?.('[data-practice-run][data-timed="true"]'),timer=form?.closest('main,body')?.querySelector?.('[data-practice-timer]')||root.querySelector?.('[data-practice-timer]');if(!form||!timer)return;let left=Number(timer.textContent||60);timerId=setInterval(()=>{left=Math.max(0,left-1);timer.textContent=String(left);if(left<=0){clearInterval(timerId);timerId=null;form.requestSubmit()}},1000)}
