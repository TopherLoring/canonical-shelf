...
export function homeView({data,state,esc}){
  const s=progressSummary(data,state),practice=practiceStateSummary(),next=s.next,nextUnit=next&&data.units?.find(u=>u.id===next.unitId),nextCourse=next&&data.courses?.find(c=>c.id===next.courseId);
