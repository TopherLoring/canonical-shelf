...
requireText(index,'/canonical-shelf.css','canonical visual contract not loaded');
forbid(index,'/tokens.css','obsolete token stylesheet must not be a loaded visual authority');
...
for(const footerHref of ['/about.html#faith','/about.html','/privacy.html','/data-retention.html','/storage.html','/terms.html','/safety.html'])requireText(index,`href="${footerHref}"`,`footer destination missing: ${footerHref}`);
for(const disclosure of ['Statement of Faith','How Canonical Shelf approaches Scripture','Sources &amp; methodology','Translation information','Accessibility','Privacy'])requireText(about,disclosure,`About surface missing disclosure: ${disclosure}`);
...
for(const marker of ['library-first-home','library-first-homehead','library-first-shelf-row','Old Testament','New Testament','library-first-bookend','Browse books','Ask a question','Learn in sequence','Retain what matters','Continue learning','Current context','Recent study'])requireText(home,marker,`Home shelf-first surface missing: ${marker}`);
for(const marker of ['--home-bg:#252a30','--home-shelf-height:111px','--home-shelf-depth:10px','library-first-homehead'])requireText(contract,marker,`Home locked visual contract missing: ${marker}`);

for(const marker of ['courseLandingView','courseDetailView','unitExperienceView','course-volume-landing','course-volume-shelf','course-catalog-detail','Current','Next','Later','review','mastery'])requireText(course,marker,`Course contract missing: ${marker}`);
...
for(const marker of ['bible-reader-shell','bible-reader-shelves','bible-address-tools','Books','Timeline','Maps','Search','Book Notes','library-reader-layout','library-reader-panel','data-selected','nearby verses','Bookshelf','Books & groups','Canon & timeline','data-book-drawer','library-search','shelf-spine'])requireText(bible+libraryData,marker,`Bible surface missing: ${marker}`);
const profileCount=(otBooks.match(/\{n:\d+,name:/g)||[]).length+(ntBooks.match(/\{n:\d+,name:/g)||[]).length;
if(profileCount!==66)throw new Error(`expected 66 Bible book profiles, found ${profileCount}`);
...
for(const marker of ['Questions','Theology & doctrine','Christian life','Biblical concepts','Difficult questions','Glossary','Related exploration','topics-dossier','topics-dossier__questions','topicSearchText','courseConnections','Scripture connections','Course connections','Related search language','t.sections','t.refs','t.aliases'])requireText(topicsExperience,marker,`Topics depth contract missing: ${marker}`);
...
for(const marker of ['queryStudyIndex','topicIndexText','lessonIndexText','bookIndexText','verseIndexText','topic.sections','topic.aliases','topic.refs','lesson.body','lesson.vocab','lesson.drawers','LIBRARY_BOOKS','VERSES','topicEvidenceDetail','lessonEvidenceDetail','bookEvidenceDetail','verseEvidenceDetail'])requireText(studyIndex,marker,`Shared study index missing corpus field: ${marker}`);
for(const marker of ['searchCanonicalShelf','searchExperienceView','queryStudyIndex','Scripture','Topics','Course','Book profiles','Curated passages','Glossary','Ask the Theologian with the evidence in view'])requireText(searchExperience,marker,`Unified Search presentation/access contract missing: ${marker}`);
...
for(const marker of ["from './study-index.js'",'queryStudyIndex','masteryProtection','Canonical Shelf Statement of Faith','curated reference','course instruction','book orientation','curated passage context','reference definition','scholarly source'])requireText(theologian,marker,`Theologian evidence graph/boundary contract missing: ${marker}`);
for(const marker of ['topicEvidenceDetail','lessonEvidenceDetail','bookEvidenceDetail','verseEvidenceDetail','/topics?topic=','/course?unit=','/bible?book=','/practice?mode=verses','/topics?mode=glossary'])requireText(theologian,marker,`Theologian evidence handoff missing: ${marker}`);
...
for(const asset of ['/home.html','/course.html','/bible.html','/topics.html','/practice.html','/search.html','/about.html','/canonical-shelf.css','/footer.css','/about-page.js','/experience.js','/topics-experience.js','/topics-experience.css','/search-experience.js','/search-experience.css','/study-index.js','/progress-experience.js','/course-experience.js','/learning-visuals.js','/learning-visuals.css','/bible-state.js','/bible-state.css','/library-data.js','/library-books-ot.js','/library-books-nt.js','/practice-experience.js','/practice-engine.js','/practice-engine-restored.js','/practice-state.js','/practice-data.js','/verse-data.js','/verse-data-01.js','/verse-data-08.js'])requireText(sw,asset,`offline shell missing restored asset: ${asset}`);
...
for(const marker of ['routeFromPath','location.assign(targetPath)','targetRoute===route()'])requireText(app,marker,`native cross-destination routing contract missing: ${marker}`);
...
console.log(`experience contract passed: locked Home/Course/Bible/Topics/Practice destinations, 66 book profiles with learner state, shared Search/Theologian evidence graph, ${restoredVerseCount} curated passages, native Practice engines, route-owned documents, and semantic Course visuals`);
