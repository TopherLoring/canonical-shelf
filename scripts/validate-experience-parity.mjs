...
for(const footer of ['site-footer','Statement of Faith','About Canonical Shelf','How Canonical Shelf approaches Scripture','Sources &amp; methodology','Translation information','Accessibility','Privacy'])requireText(index,footer,`footer/about missing: ${footer}`);
...
for(const marker of ['library-first-home','library-first-shelf','A Bible-learning library','66-book library','Browse books','Theologian','6 courses','Reviews &amp; games','Continue learning','Current context','Recent study','practiceStateSummary','XP','achievements'])requireText(home,marker,`Home Library First surface missing: ${marker}`);
for(const marker of ['courseLandingView','courseDetailView','unitExperienceView','course-catalog-chooser','course-catalog-detail','Current','Next','Later','review','mastery'])requireText(course,marker,`Course parity contract missing: ${marker}`);
...
console.log(`cross-tab restoration contract passed: 66 book profiles with learned/current state, full Topics depth, shared Search/Theologian evidence graph, ${restoredVerseCount} restored curated passages, native Practice engines, route-owned documents, and 12 semantic Course visual families`);
