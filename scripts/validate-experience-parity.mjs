...
for(const marker of ['searchCanonicalShelf','searchExperienceView','queryStudyIndex','Scripture','Topics','Course','Book profiles','Curated passages','Glossary','Ask the Guide with the evidence in view'])requireText(searchExperience,marker,`Unified Search presentation/access contract missing: ${marker}`);
...
for(const marker of ["from './study-index.js'",'queryStudyIndex','masteryProtection','Canonical Shelf Statement of Faith','curated reference','course instruction','book orientation','curated passage context','reference definition','scholarly source'])requireText(theologian,marker,`Guide evidence graph/boundary contract missing: ${marker}`);
for(const marker of ['topicEvidenceDetail','lessonEvidenceDetail','bookEvidenceDetail','verseEvidenceDetail','/topics?topic=','/course?unit=','/bible?book=','/practice?mode=verses','/topics?mode=glossary'])requireText(theologian,marker,`Guide evidence handoff missing: ${marker}`);
requireText(app,'e.href||fallback','Guide evidence renderer must honor owning-surface links');
for(const boundary of ['scored activity','prohibited overstatement','arsenokoitai','malakoi','Romans 1'])requireText(theologian,boundary,`Guide safety/editorial boundary missing: ${boundary}`);
