...
if(!buildVerify.includes('repair:prelaunch')||!buildVerify.includes('build:runtime')||!buildVerify.includes('verify:bsb')||!buildVerify.includes('validate:llms'))throw new Error('prelaunch verification build must repair deterministic content, compile runtime, verify BSB integrity, and validate llms.txt');
...
for(const path of [
  'scripts/generate-curriculum-reference.mjs',
...
console.log('production build/deploy separation + current curriculum/llms generation + prelaunch/full gate split + immutable BSB integrity passed');
