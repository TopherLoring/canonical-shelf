...
const ROUTE_DESCRIPTIONS={
  ...
  course:'Guided curriculum and scored learning activities.',
...
const PUBLIC_RESOURCES=[
  ['Curriculum reference','/data/curriculum.md','Generated curriculum reference for the current guided course.'],
  ['Runtime catalog','/data/catalog.json','Machine-readable units, lessons, mastery activities, Topics, and stable activity identifiers.'],
...
const OPTIONAL_RESOURCES=[
  ['Bible corpus','/data/corpus.txt','Embedded Scripture corpus used by Bible reading and search; large file, fetch only when needed.']
];
...
export function parsePrimaryNavigation(html){
  ...
  for(const match of nav.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)){
    ...
    const route=attr(attributes,'data-route');
    if(!route)continue;
...
export function summarizeCatalog(catalog){
  ...
  const masteryActivities=Array.isArray(catalog?.masteryIds)?catalog.masteryIds.length:
    Array.isArray(catalog?.activities)?catalog.activities.filter(activity=>activity?.type==='mastery').length:0;
  ...
  if(!units||!guidedLessons||!masteryActivities||!scoredActivities)throw new Error('runtime catalog is incomplete; run migration before generating llms.txt');
  ...
  return {units,guidedLessons,masteryActivities,scoredActivities,topics};
}

export function renderLlms({routes,counts}){
  const lines=[
    '# Canonical Shelf','',
    '> Canonical Shelf is an offline-capable Bible-learning and scholarly reference application combining guided curriculum, Scripture study, curated Topics, Practice, and an evidence-aware study Guide.','',
    ...
    `Current scored curriculum: ${counts.units} units, ${counts.guidedLessons} guided lessons, ${counts.masteryActivities} mastery activities, and ${counts.scoredActivities} scored activities. Current reference library: ${counts.topics} Topics.`,'',
...
export async function buildLlmsContract({root=process.cwd()}={}){
  ...
  const routes=parsePrimaryNavigation(html);
  const catalog=JSON.parse(catalogText);
  const counts=summarizeCatalog(catalog);
  const resources=[...PUBLIC_RESOURCES,...OPTIONAL_RESOURCES];
