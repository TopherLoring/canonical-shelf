...
for(const heading of ['## Primary destinations','## Canonical learning and editorial resources','## Institutional disclosures','## Excluded full-text resource','## Complete learner-facing content'])if(!output.includes(heading))fail(`missing ${heading}`);
...
console.log(`llms.txt complete-learner-corpus/freshness/curated-BSB/full-BSB-exclusion/discovery gates passed (${contract.learnerSummary.books} books, ${contract.learnerSummary.practiceLevels} Practice levels, ${contract.learnerSummary.curatedPassages} curated passages, ${contract.learnerSummary.themes} themes)`);
