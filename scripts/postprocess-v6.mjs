...
function adaptChallenge(ch){if(!ch)return ch;if(ch.kind==='argument'&&Array.isArray(ch.items)&&Array.isArray(ch.answer)){return {...ch,kind:'argument-map',fields:ch.answer.map(([src])=>ch.items[src]),options:ch.answer.map(()=>ch.items),answer:ch.answer.map(([,target])=>target)}}return ch}
cat.units=units;
cat.lessons=cat.lessons.map((l,order)=>{const legacy=v4Unit(l);return {...l,challenges:(l.challenges||[]).map(adaptChallenge),reviewChallenges:(l.reviewChallenges||[]).map(adaptChallenge),v4Unit:legacy,v6Unit:mapLesson({...l,v4Unit:legacy}),v6Order:order}});
