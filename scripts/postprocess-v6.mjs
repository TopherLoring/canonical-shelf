...
// This recovers the current v7 semantic unit used to place the 70 inherited lessons.
// It is intentionally preserved so the migration can distinguish old unit meaning from
// the new six-course placement without changing any stable lesson IDs.
...
const legacyLessons=cat.lessons.map((lesson,order)=>{
  ...
  return {
    ...lesson,
    challenges:(lesson.challenges||[]).map(adaptChallenge),
    reviewChallenges:(lesson.reviewChallenges||[]).map(adaptChallenge),
    v4Unit:legacy,
    v6Unit:unitId,
    unitId,
    courseId:courseForUnit(unitId),
    legacyGuided:true,
    legacyOrder:order
  };
});
...
const addedLessons=newLessons.map((lesson,order)=>({
  ...lesson,
  challenges:(lesson.challenges||[]).map(adaptChallenge),
  reviewChallenges:(lesson.reviewChallenges||[]).map(adaptChallenge),
  v6Unit:lesson.unitId,
  courseId:courseForUnit(lesson.unitId),
...
for(const [legacyUnit,ids] of Object.entries(cat.legacyMasteryPlacement||{})){
  for(const id of ids){
    const unitId=mapLegacyUnit(legacyUnit);
    legacyMasteryActivities.push({
      id:`mastery:${id}`,
      type:'mastery',
      masteryType:'legacy',
      unitId,
      courseId:courseForUnit(unitId),
      title:cat.legacyMastery?.CANON_V4_MASTERY?.[id]?.title||id,
      sourceId:id
    });
  }
}

const lessonActivities=cat.lessons.map(lesson=>({
  id:`lesson:${lesson.id}`,
  type:'lesson',
  ...
  title:lesson.title,
  sourceId:lesson.id,
  legacy:!!lesson.legacyGuided
}));

const newMasteryActivities=Object.values(newMastery).map(item=>({
  id:`mastery:${item.id}`,
  type:'mastery',
  masteryType:item.type,
  unitId:item.unitId,
...
const preferredLessonOrder={
  'c1.christianity':['begin'],
  'c1.bible':['library','shelf-skills'],
  'c1.transmission':['c1-bible-languages','translation'],
  'c1.reading':['context'],
  'c1.theology':['c1-theology-map'],
  'c1.practice':['c1-practice-map'],
  'c1.traditions':['c1-traditions-map'],
  'c1.story':['story','garden-trust','sin-mortality','creation-care','abraham-blessing'],
  'c2.exodus':['exodus-call','c2-passover','c2-sea-wilderness'],
  'c2.sinai':['c2-sinai-covenant','c2-golden-calf','torah-neighbor'],
  'c2.tabernacle':['c2-tabernacle','c2-ark'],
  'c2.sacrifice':['c2-priesthood-sacrifice','c2-day-atonement','c2-sacred-calendar'],
  'c2.land-kings':['c2-davidic-covenant'],
  'c2.temple-kingdom':['c2-temple-presence'],
  'c2.prophets-exile':['c2-exile-temple-loss'],
  'c2.restoration-hope':['c2-restoration-temple','c2-new-covenant','c2-prophetic-hope'],
  'c3.after-exile':['c3-persian-judea','c3-torah-public-life'],
  'c3.greek-world':['c3-alexander-hellenization','c3-antiochus-maccabees'],
  'c3.hasmonean-rome':['c3-hasmoneans','c3-rome-judea','c3-herod'],
  'c3.jewish-life':['c3-temple-priesthood','c3-synagogue-diaspora','c3-pharisees-sadducees','c3-essenes-qumran','c3-revolutionary-currents'],
  'c3.expectation':['c3-apocalyptic-resurrection','c3-messianic-diversity'],
  'c3.enter-gospels':['c3-enter-gospels'],
  'c4.gospels':['gospel-comparison'],
  'c4.teaching':['c4-sermon-mount','prayer-practice'],
  'c4.israel-story':['c4-scripture-fulfillment'],
  'c4.passion':['c4-passion-passover','communion-table','c4-ascension'],
  'c4.pentecost':['c4-pentecost'],
  'c4.paul-gentiles':['acts-discernment'],
  'c4.expansion':['body-belonging','letters-favoritism'],
  'c5.translation':['c5-equivalence'],
  'c5.genre':['c5-hebrew-poetry'],
  'c5.intertext':['c5-typology-allusion'],
  'c5.gospel-letters':['c5-synoptic-problem','c5-authorship-composition'],
  'c6.god-christ':['trinity-language'],
  'c6.sin-salvation':['atonement-images','receiving-grace','grace-repair'],
  'c6.providence-life':['providence-freedom'],
  'c6.church-practice':['baptism-belonging'],
  'c6.traditions':['traditions-welcome','c6-covenant-frameworks'],
  'c6.difficult':['ethical-texts'],
  'c6.final-hope':['apocalypse-hope','c6-end-times-frameworks']
};

const activityById=Object.fromEntries(cat.activities.map(activity=>[activity.id,activity]));
function lessonRank(activity){
  if(activity.type!=='lesson')return Number.MAX_SAFE_INTEGER;
  const preferred=preferredLessonOrder[activity.unitId]||[];
  const index=preferred.indexOf(activity.sourceId);
  if(index>=0)return index;
  const lesson=cat.lessons.find(item=>item.id===activity.sourceId);
  const fallbackBase=preferred.length+100;
...
cat.byCourse=Object.fromEntries(courses.map(course=>[
  course.id,
  units.filter(unit=>unit.courseId===course.id).map(unit=>unit.id)
]));

for(const activity of cat.activities){
  if(!activityById[activity.id])throw new Error(`activity index missing ${activity.id}`);
  if(!cat.byUnit[activity.unitId]?.includes(activity.id))throw new Error(`activity ${activity.id} missing from unit placement`);
}

// Build a reusable glossary from authored vocabulary. Existing lesson vocabulary remains canonical;
// repeated terms accumulate occurrences instead of silently replacing definitions.
...
// Structural assertions replace obsolete fixed curriculum ceilings.
